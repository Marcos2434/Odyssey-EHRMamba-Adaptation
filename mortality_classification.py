...
class SimpleClassifier(nn.Module):
    def __init__(self):
        ...
        # Definimos una capa lineal, pero no especificamos el tamaño del sensor_count en el constructor
        self.fc = nn.Linear(0, 2)  # Inicialmente el tamaño es 0, lo actualizamos en forward

    def forward(self, logits_pooling):
        # Detectamos automáticamente la dimensión de entrada (sensor_count) desde logits_pooling
        sensor_count = logits_pooling.size(1)
        
        # Redefinimos la capa lineal con el tamaño correcto
        self.fc = nn.Linear(sensor_count, 2)
        
        # Pasamos los logits a través de la capa lineal para obtener las predicciones
        output = self.fc(logits_pooling)
        return output
...
def train(
    ...
    **kwargs,  
    ...
    for epoch in range(epochs):
        ...
        #     # Si el batch es una tupla o una lista
        #     if isinstance(batch, (tuple, list)):
        #         for i, element in enumerate(batch):
        #             if isinstance(element, torch.Tensor):
        #                 print(f"Batch[{i}] Tensor: shape = {element.shape}, dtype = {element.dtype}")
        #             else:
        #                 print(f"Batch[{i}] Type: {type(element)}")
        #     # Si el batch es un diccionario
        #     elif isinstance(batch, dict):
        #         for key, value in batch.items():
        #             if isinstance(value, torch.Tensor):
        #                 print(f"Batch['{key}'] Tensor: shape = {value.shape}, dtype = {value.dtype}")
        #             else:
        #                 print(f"Batch['{key}'] Type: {type(value)}")
        #     else:
        #         print("Unknown batch structure:", type(batch))
        #     break  # Salimos tras el primer lote
        for batch in tqdm.tqdm(train_dataloader, total=len(train_dataloader)):
            # def inspect_batch(batch):
            #     def inspect_element(element, prefix=""):
            #         if isinstance(element, torch.Tensor):
            #             print(f"{prefix}Tensor: shape = {element.shape}, dtype = {element.dtype}")
            #         elif isinstance(element, (list, tuple)):
            #             print(f"{prefix}{type(element).__name__}: length = {len(element)}")
            #             for i, sub_elem in enumerate(element):
            #                 inspect_element(sub_elem, prefix + f"  [{i}] ")
            #         elif isinstance(element, dict):
            #             print(f"{prefix}Dict: keys = {list(element.keys())}")
            #             for key, sub_elem in element.items():
            #                 inspect_element(sub_elem, prefix + f"  ['{key}'] ")
            #         else:
            #             print(f"{prefix}Unknown: type = {type(element)}")

            #     print("Inspecting batch:")
            #     inspect_element(batch)

            # # Ejemplo de uso:
            # batch = (
            #     torch.randn(32, 10, 5),  # Tensor 1
            #     {"static": torch.randn(32, 3), "time": torch.arange(32)},  # Diccionario
            #     [torch.ones(10, 10), torch.zeros(5, 5)],  # Lista
            # )
            ...
            if model_type == "EHRMamba":
                logits = predictions.logits.squeeze(-1)
                
                logits_pooling = torch.median(logits, dim=1)[0]

                classifier_model = SimpleClassifier()

                predictions = classifier_model(logits_pooling)
            else:

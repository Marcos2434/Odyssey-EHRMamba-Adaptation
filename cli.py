...
@click.option("--state_size", default=64, help="State size for Mamba")
def core_function(
    output_path,
    base_path,
    model_type,
    ...
    **kwargs
    ...
    if not model_type == "EHRMamba": model_args = kwargs
    else:
        # Only accept these options for the EHRMamba model
        model_args = dict()
        options = [
            'vocab_size',
            'max_num_visits',
        ...
        for o in options:
            if o in kwargs: model_args[o] = kwargs[o]
    ...
    for split_index in range(1, 6):
        ...
        model_settings = {
            ...
            "base_path": base_path,
            "pooling_fxn": model_args["pooling"] if not model_type == "EHRMamba" else None,
        }
        ...
        if model_type == "EHRMamba":
            # TODO: Settings for EHRMamba Model

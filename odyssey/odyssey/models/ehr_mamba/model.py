...
from odyssey.odyssey.models.ehr_mamba.mamba_utils import (
    MambaForMultiHeadSequenceClassification,
    MambaForSequenceClassification,
    MambaSequenceClassifierOutput,
)
## from odyssey.odyssey.models.embeddings import MambaEmbeddingsForCEHR # igual quitando esto va más rapido
from odyssey.odyssey.models.embeddings import MambaEmbeddingsForCEHR_adapted

# ## funcoin de embedding rapida y sencilla
# def simple_embedding(x, static, time, sensor_mask):
#     # La salida esperada tiene las dimensiones [128, 158, 74]
#     batch_size = x.size(0)  # 128 en este caso

#     # Aquí generamos datos aleatorios para simular un proceso de embedding rápido
#     # Ajustamos el tamaño de salida a [batch_size, 158, 74]
#     random_embeds = torch.randn(batch_size, 158, 74)  # Generación rápida de datos aleatorios

#     return random_embeds

# # Modelo simple y tonto para borrar
# class FastRandomModel(nn.Module):
#     def __init__(self, input_dim=74, output_dim=37):
#         super(FastRandomModel, self).__init__()
#         # Capa lineal para mapear directamente las dimensiones de entrada a salida.
#         self.fc = nn.Linear(input_dim, output_dim)

#     def forward(self, inputs_embeds):
#         # Mapea los inputs directamente a los logits
#         logits = self.fc(inputs_embeds)
        
#         # Empaqueta los logits en el formato deseado
...
class MambaPretrain(pl.LightningModule):
    ...
    def __init__(
        self,
        vocab_size: int,
        embedding_size: int = 74,
        time_embeddings_size: int = 16,
        visit_order_size: int = 2,
        type_vocab_size: int = 9,
        max_num_visits: int = 124,
        ...
        self.embeddings = MambaEmbeddingsForCEHR_adapted(
            config=self.config,
        ...
        #self.model = FastRandomModel() #eliminar esto luegooo
    ...
    def forward(
        self, x, static, time, sensor_mask, delta, **kwargs,
        # labels: Optional[torch.Tensor] = None,
        # output_hidden_states: Optional[bool] = False,
        # return_dict: Optional[bool] = True,
    ) -> Union[Tuple[torch.Tensor, ...], MambaCausalLMOutput]:
        """Forward pass for the model."""
        # print("------ start forward() function in model.py-------")
        ...
        # inputs_embeds = simple_embedding(
        #     x=x,
        #     static=static,
        #     time=time,
        #     sensor_mask = sensor_mask,
        # )
        # inputs_embeds = torch.rand(8052, 152, 74)
        # print("INPUTS EMBEEEDS")
        #print(inputs_embeds)
        # print(inputs_embeds.shape)

        return self.model(
           inputs_embeds=inputs_embeds,
            #labels=labels, look this!!
            #output_hidden_states=output_hidden_states,
            #return_dict=return_dict,
        )

# creo que a partir de aqui se puede borrar todo
    ...
    def validation_step(self, batch: Dict[str, Any], batch_idx: int) -> Any:
        ...
        print("------VALIDATION STEP USANDOSE---------")
        labels = batch["labels"]

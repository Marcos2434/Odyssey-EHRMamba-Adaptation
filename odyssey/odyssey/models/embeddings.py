...
class MambaEmbeddingsForCEHR_adapted(nn.Module):
    """Construct the embeddings from multiple input sources: data, static, time, etc."""
    ...
    #     hidden_dropout_prob: float = 0.1,
    # ) -> None:
    #     """Initialize embedding layers and parameters for the new data structure."""
    #     super().__init__()
    #     self.type_vocab_size = type_vocab_size
    #     self.max_num_visits = max_num_visits
    #     self.layer_norm_eps = layer_norm_eps
    ...
    #     # Embeddings for dynamic sequence data
    #     self.word_embeddings = nn.Embedding(
    #         config.vocab_size,
    #         config.hidden_size,
    #         padding_idx=config.pad_token_id,
    ...
    #     # Embeddings for time and static features
    #     self.time_embeddings = TimeEmbeddingLayer(
    #         embedding_size=time_embeddings_size,
    #         is_time_delta=True,
    #     )
    ...
    #     # Optional additional embeddings (e.g., mask or delta)
    #     self.mask_delta_embeddings = nn.Linear(
    #         time_embeddings_size,  # Adjust size as needed for mask/delta representation
    #         config.hidden_size,
    #     )

    #     # Layer to combine multiple embeddings
    #     self.scale_back_concat_layer = nn.Linear(
    #         config.hidden_size + 2 * time_embeddings_size + config.hidden_size,
    #         config.hidden_size,
    #     )

    #     # Non-linearity, normalization, and dropout
    #     self.tanh = nn.Tanh()
    #     self.LayerNorm = nn.LayerNorm(config.hidden_size, eps=self.layer_norm_eps)
    #     self.dropout = nn.Dropout(self.hidden_dropout_prob)
    ...
    #     inputs: tuple,
    # ) -> Any:
    #     """Return the final embeddings for the tuple-based input data.

    #     Parameters
    #     ----------
    #     inputs : tuple
    #         Tuple of input data structured as:
    #         (data, static, time, labels, mask_delta)
        
    #     Returns
    #     -------
    #     torch.Tensor
    #         The final embeddings after processing all input components.
    #     """
    #     # Unpack tuple
    #     data, static, time, labels, mask, delta = inputs

    #     # Step 1: Process sequence data embeddings
    #     data_embeds = self.word_embeddings(data)

    #     # Step 2: Process time data embeddings
    #     time_embeds = self.time_embeddings(time)
        
    #     # Step 3: Process static data embeddings
    #     static_embeds = self.static_embeddings(static)

    #     # Step 4: Process additional features (mask and delta)
    #     mask_delta_embeds = self.mask_delta_embeddings(mask_delta)

    #     # Step 5: Concatenate embeddings from different sources
    #     combined_embeds = torch.cat(
    #         (data_embeds, time_embeds, static_embeds, mask_delta_embeds),
    #         dim=-1,
    #     )
    ...
    #     # Step 6: Add positional and token type embeddings
    #     embeddings = combined_embeds + token_type_embeds

    #     # Step 7: Apply dropout and normalization
    #     embeddings = self.dropout(embeddings)
    #     embeddings = self.LayerNorm(embeddings)
